...
from train import EPOCH_COUNT, VER_NUMBER

SHOW_CONFUSION_MATRIX = True
def validate():

    #test_ds = "data/UWTempo/ver13/testing/testing12.2.bin"
    #ds = LabeledStateDataset(test_ds)
    combined_ds = load_dataset_from_directory(f"data/UWTempo/ver{VER_NUMBER}/testing")
    with open("ignore.roar", "rb") as f:
        ignore = BitMap.deserialize(f.read())
    #ignore = create_redundancy_ignore_list(combined_ds, train.GLOBAL_MAX)
    #ignore = set()
    print(len(ignore))
    ...
    for i in range(1, train.EPOCH_COUNT+1):
        checkpoint_path = f"models/model{VER_NUMBER}/ckpt_{i}.pt"  # Make sure this is the correct checkpoint
        try:
